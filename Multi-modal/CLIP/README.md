...
대규모 이미지-텍스트 데이터셋을 활용해 자연어를 지도 신호로 삼는 CLIP은 기존 비전 모델의 한계를 극복하고 제로샷 예측을 비롯한 강력한 일반화·강건성 능력을 보여주었다. 이를 통해 멀티모달 모델 시대가 가속화되었지만, 동시에 사회적 편향과 윤리적 문제가 노출될 위험이 크다. 웹 데이터에 내재된 편향이나 개인정보 침해, 감시 등에 활용될 소지가 있어, 안전장치 마련과 책임 있는 사용이 필수적이다. 그럼에도 불구하고 CLIP은 자연어-시각 정보 결합이 컴퓨터 비전에 새로운 가능성을 열었다는 점에서 큰 의의를 지닙니다.
