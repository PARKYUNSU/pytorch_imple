...
- Closed-set 작업 (PubHealth, ARC-Challenge)
정해진 정답이 있는 데이터셋을 사용하여, 모델이 정답을 얼마나 정확하게 맞추는지(정확도)를 측정합니다. 이 경우, 모델의 출력이 정확히 정답과 일치하는지를 평가합니다.

- Short-form 생성 작업 (PopQA, TriviaQA-unfiltered):
짧은 형태의 응답을 생성하는 작업으로, 모델이 응답 내에 정답을 포함하고 있는지 확인합니다. 정답의 일부만 포함되어도 평가할 수 있어, 정확한 단어 일치보다는 정답 포함 여부를 중점으로 평가합니다.

- Long-form 생성 작업 (biography 생성, 장문 QA):
긴 텍스트를 생성하는 작업으로, 생성된 텍스트의 사실성(FactScore), 유창성(MAUVE), 그리고 응답 내 인용(인용 정밀도 및 재현율) 등의 다양한 지표를 사용하여 모델의 전반적인 품질을 분석합니다.

이 작업은 모델이 얼마나 풍부하고 정확한 정보를 제공하는지를 평가합니다.

각 데이터셋의 특징과 평가 기준에 대해 간략하게 설명함으로써, SELF-RAG의 다양한 작업에 대한 적용 가능성을 제시합니다.

## 4.2. Baselines

SELF-RAG의 성능을 비교하기 위해 두 가지 범주의 Baseline 모델을 선정했습니다.

하나는 검색 없이 사용하는 모델으로, Llama, Alpaca, ChatGPT 등이 있으며, 다른 하나는 검색 보강 모델으로, 표준 RAG 기반 모델, Llama2-FT, Ret-ChatGPT 등이 포함됩니다.

각 모델의 주요 특징과 적용 방식, 그리고 SELF-RAG와의 차별점을 중심으로 비교 분석합니다.

## 4.3. Experimental Settings

실험 설정에서는 사용한 훈련 데이터, 모델 구성 및 하이퍼파라미터, 그리고 추론 방법에 대해 상세히 기술합니다.

훈련 데이터는 다양한 지침-출력 쌍으로 구성되었으며, 데이터셋 출처와 샘플 수를 명시합니다. 

모델 구성에서는 Llama2-7B/13B와 같은 기반 모델, Beem Search 및 임계값 설정 등의 세부 사항을 포함하며, 추론 시에는 검색 빈도와 토큰 디코딩 방법 등이 적용되었습니다.
