...
# 상수들 정의
VOCAB_SIZE = 50257  # tiktoken 기준 예시
CONTEXT_LENGTH = 128
EMB_DIM = 768
NUM_HEADS = 12
NUM_LAYERS = 12
...
class TransformerBlockPreNorm(nn.Module):
    ...
    def forward(self, x):
        x = x + self.dropout(self.att(self.norm1(x)))  # pre-norm for attention
        x = x + self.dropout(self.ff(self.norm2(x)))     # pre-norm for FFN
        return x
...
class GPTModel(nn.Module):
    def __init__(self):
        ...
        self.trf_blocks = nn.Sequential(*[TransformerBlockPreNorm() for _ in range(NUM_LAYERS)])
