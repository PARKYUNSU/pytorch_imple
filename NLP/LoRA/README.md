...
$$
\max_{\Phi} \sum_{(x,y) \in Z} \sum_{t=1}^{|y|} \log P_\Phi(y_t \mid x, y_{<t}) \quad \cdots (1)
$$
...
$$
\max_{\Theta} \sum_{(x,y) \in Z} \sum_{t=1}^{|y|} \log P_{\Phi_0 + \Delta\Phi(\Theta)}(y_t \mid x, y_{<t}) \quad \cdots (2)
$$
