...
가정
| “모델이 새로운 작업에 적응할 때, 가중치 변화(ΔW) 자체도 저랭크(low-rank) 구조를 가진다.”
...
- 초기화:
  ...
  \Rightarrow \text{학습 시작 시 } \Delta W = 0
  \]

# 2. Problem Statement
 사전 학습된 언어모델(ex.Transformer기반 GPT) $P_Φ(y | x)$(파라미터 $Φ$)가 있다고 가정.
 다운스트림 작업(요약, MRC(기계독해), NL2SQL(자연어→SQL) 등)은 컨텍스트-타킷 쌍인 데이터셋 $Z = { (x_i, y_i) }$로 주어집니다.

## 2.1. Full fine-tuning
모델을 사전학습 가중치 $Φ_0$로 초기화하고, 아래 조건부 언어모델링 목적함수를 최대화하도록 $Φ$를 업데이트합니다.
$$Where$$
$$
- P_Φ(y | x):  사전학습 언어모델(autoregressive)
- Z = { (x_i, y_i) }:  컨텍스트-타깃 쌍 데이터셋
- y_<t:        시점 t 이전의 타깃 토큰들
- Φ_0:         사전학습 가중치
- ΔΦ:          작업별 가중치 증가분
- Θ:           ΔΦ를 부호화하는 소수(小數) 파라미터 집합 (|Θ| << |Φ_0|)
$$


$$(1)  maximize over Φ:
     sum_{(x,y) ∈ Z} sum_{t=1}^{|y|}
       log P_Φ( y_t | x, y_<t )
$$
|단점: 작업마다 $ΔΦ$를 따로 학습, 저장해야 하고, 그 크기 $|ΔΦ|$가 $|Φ_0|$와 동일함. 대형 모델일수록(ex. GPT-3 175B) 여러 작업 인스턴스를 배포하기 어렵거나 비현실적.

## 2.2. PEFT(Parameter-Efficient Fine-Tuning)
작업별 증가분 $ΔΦ$를 **작은 파라미터 집합 $Θ$**로 부호화합니다. 즉, $ΔΦ = ΔΦ(Θ)$이며 $|Θ| << |Φ_0|$. 이제 최적화 대상은 Φ가 아니라 Θ입니다.
$$

(2)  maximize over Θ:
     sum_{(x,y) ∈ Z} sum_{t=1}^{|y|}
       log P_{ Φ_0 + ΔΦ(Θ) }( y_t | x, y_<t )
$$
|이때 $ΔΦ$는 저랭크(low-rank) 표현으로 구성해 연산/메모리 효율을 얻습니다. 예를 들어 GPT-3 175B 기준으로 학습 파라미터 수 $|Θ|$를 $|Φ_0|$의 약 0.01%까지 줄일 수 있습니다.
...
Adapter는 파라미터 수가 작지만, 추론 속도를 느리게 만듭니다.
각 Transformer block 마다 adapter층을 추가하면, 계산 순서가 깊어져서 병렬화가 안 됨, 특히 실시간 inference(Batch size=1)에서는 속도가 지연됨.
