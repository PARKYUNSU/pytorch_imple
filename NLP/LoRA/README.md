...
LoRA는 LLM을 효율적으로 Fine-tuning하기 위한 방법으로, 전통적인 Fine-tuning과 다르게 기존 사전학습 모델의 가중치는 고정시키고, 각 층(Layer)에 저랭크 행렬(rank decomposition matrices)만 추가 학습합니다.

- 원래 가중치 $W_0$는 **고정(frozen)**

- 대신 $W = W_0 + \Delta W$ 형태로 학습

- $\Delta W = B A$ (저랭크 행렬 분해, rank $r$)

- $W_0 \in \mathbb{R}^{d_{\text{out}} \times d_{\text{in}}}$, $A \in \mathbb{R}^{r \times d_{\text{in}}}$, $B \in \mathbb{R}^{d_{\text{out}} \times r}$
...
## 1.3. 핵심 직관: 저차원 고유 순위 가설 (The Low-Rank Intrinsic Rank Hypothesis)

LoRA의 출발점은 **"거대 언어 모델을 새로운 작업에 적응시킬 때, 가중치의 변화량($\Delta W$)은 그 행렬의 거대한 차원에도 불구하고, 본질적으로는 매우 낮은 '고유 순위(intrinsic rank)'를 가질 것이다"**라는 가설입니다.

### 백과사전 비유

이것이 무슨 의미일까요? 쉽게 비유해 보겠습니다. 수천 페이지에 달하는 방대한 백과사전(사전 훈련된 모델)이 있다고 상상해 봅시다. 이 백과사전에 '최신 스마트폰 기술'이라는 새로운 주제를 추가하기 위해 백과사전 전체를 처음부터 다시 쓸 필요는 없습니다. 대신, 몇 페이지 분량의 부록(appendix)을 만들어 기존 내용과 연결하면 충분합니다. 

- **백과사전 전체**: 고차원(high-dimension)의 원본 지식($W_0$)
- **부록**: 저차원(low-dimension)의 핵심적인 변화량($\Delta W$)

LoRA는 이 '부록'이 매우 얇고 간결할 것이라고 가정하고, 이 부록을 만드는 가장 효율적인 방법을 찾아낸 것입니다.
...
> Adapter는 레이어 깊이를 늘려 순차 경로가 추가되므로, 특히 배치=1 온라인 추론/모델 샤딩 환경에서 지연(latency)이 체감 증가할 수 있습니다.

## 3.3. Prompt Optimization 문제점
입력 임베딩을 직접 학습하는 Prompt Tuning은 다음과 같은 문제점이 있습니다.
...
> 프롬프트 튜닝은 최적화 난이도와 유효 입력 길이 감소 이슈가 보고되지만, 효과는 태스크/모델/길이 제약에 따라 크게 달라질 수 있습니다.
...
### 학습 파라미터의 극적인 감소
앞서 설명했듯이, GPT-3 175B 모델의 경우 1750억 개의 파라미터를 약 1800만~3500만 개 수준으로 줄여, 일반적인 환경에서도 미세조정이 가능해집니다.

### GPU 메모리 요구량 감소
학습 대상 파라미터가 줄어들면서 그래디언트와 옵티마이저 상태에 필요한 메모리도 함께 감소합니다. 그 결과, GPT-3 훈련 시 GPU VRAM 사용량을 1.2TB에서 350GB로 약 3배(2/3) 줄이는 효과를 가져왔습니다.

### 작고 가벼운 체크포인트 (Checkpoint)
미세조정 후에는 학습된 작은 행렬 $A$와 $B$만 저장하면 됩니다. 이 덕분에 모델 체크포인트의 크기가 수백 GB에서 수십 MB 단위로 극적으로 줄어듭니다. 이는 모델의 공유, 버전 관리, 배포를 매우 용이하게 만듭니다.

### 추론 지연 없음 (No Inference Latency)
LoRA의 가장 큰 장점 중 하나입니다. 훈련이 끝나고 모델을 실제 서비스에 배포할 때는, $W' = W_0 + B \times A$ 계산을 통해 두 행렬을 하나의 새로운 가중치 행렬로 완전히 **병합(merge)**할 수 있습니다. 

이렇게 병합된 모델은 추가적인 계산이나 별도의 모듈 없이 기존 모델과 동일한 구조를 가지므로, 추론 시 어떠한 속도 저하도 발생시키지 않습니다. 이는 레이어를 추가하여 추론 지연을 유발하는 어댑터 방식과의 결정적인 차이점입니다.
...
** 장점:**
- 본체 가중치를 동결하므로 원 모델의 지식 보존에 유리
- 어댑터 제거 시 쉽게 원 상태로 복귀 가능
...
많은 사례에서 **Query ($W_q$)**와 **Value ($W_v$)** 가중치에만 적용해도 우수한 효율-성능 절충을 보입니다.
...
**확장 가능:**
- 필요 시 $W_k$, $W_o$, FFN 등 다른 선형층에도 확장 적용 가능
- 태스크/모델에 따라 다른 조합이 더 효과적일 수 있음
...
**추가 FLOPs 비율(대략)**: $r/d_{\text{in}} + r/d_{\text{out}}$ (정사각이면 $\approx 2r/d$)  
**예:** $d=12288$, $r=8$일 때, 추가 계산량은 **약 0.13%** (모델/레이어 구성에 따라 상이)

## 4.7. 추론(Inference) 시 병합 (Merge)

LoRA의 또 하나의 강점은 **추론 시 오버헤드가 최소화**된다는 것입니다.
...
> **참고**: 프레임워크/양자화 설정에 따라 미세한 차이는 있을 수 있음
...
논문에서는 흥미로운 관찰을 제시합니다: **$r=8$ 어댑터와 $r=64$ 어댑터의 상위 특이벡터들이 만드는 부분공간이 크게 겹친다**는 것입니다.

여기서 $A_{r=8}$, $A_{r=64}$는 각각 랭크 8/64로 학습된 어댑터(적응) 행렬이고, 이들의 오른쪽 특이벡터(상위 $i,j$개)가 span하는 부분공간의 유사도를 비교하여 **"작은 랭크로도 가장 중요한(상위) 방향은 잘 잡힌다"**는 것을 보였습니다.

### 핵심 발견

- **상위 특이방향의 겹침**: 특히 최상위(Top-1) 방향의 겹침이 두드러짐
- **낮은 랭크의 효과**: $r$가 작아도 핵심 방향을 잘 잡을 수 있음
- **성능의 근거**: 저랭크 어댑터도 성능이 꽤 잘 나오는 이유

### 유사도 측정

두 어댑터 간의 부분공간 유사도를 다음과 같이 측정:
...
- **상위 몇 개 방향**(예: Top-1)의 유사도가 크다는 것이 중요
- 이는 작은 $r$로도 중요한 표현 방향을 포착한다는 근거
- 예: $r=1$도 의외로 강한 성능을 보임
- **결론**: $r=1 \sim 8$ 같은 낮은 랭크도 꽤 잘 동작할 수 있다는 근거가 됩니다

> **주의**: 이 관찰은 특정 레이어(예: 48/96), 특정 모듈($W_q$, $W_v$ 등), 과업, 데이터에 따라 정도가 달라질 수 있습니다. 보편적 동등성을 주장하는 것은 아닙니다.
